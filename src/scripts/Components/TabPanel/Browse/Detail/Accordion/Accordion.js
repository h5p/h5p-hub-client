...
class Accordion extends React.Component {
  ...
  handleToggle(child) {
    this.setState({
      expanded:
        this.state.expanded === child.props.id ? undefined : child.props.id,
    });
  }
  ...
  render() {
      ...
      return (
        <div>
          ...
          >
            ...
            >
              <span className="icon-accordion-arrow" />
          ...
          <dl
            role="region"
            className={
              'h5p-hub-accordion-region' +
              (this.state.expanded == child.props.id ? '' : ' hidden')
            }
          >
    ...
    return <dl className="h5p-hub-accordion">{elements}</dl>;
  }
}
